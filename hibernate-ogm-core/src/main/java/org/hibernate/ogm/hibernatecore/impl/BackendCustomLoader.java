...
/**
 * Execute a {@link CustomQuery} representing a native NoSQL query.
 *
...
public class BackendCustomLoader extends CustomLoader {
	...
	private Iterator<Tuple> executeQuery(SessionImplementor session, DatastoreProvider provider, Type[] resultTypes) {
		...
		return provider.executeBackendQuery( session, customQuery, metadatas );
	}
	...
	private <T> OgmLoader createLoader(SessionImplementor session, Class<T> entityClass) {
