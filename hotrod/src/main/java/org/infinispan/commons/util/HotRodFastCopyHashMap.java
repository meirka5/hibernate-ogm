 ...
 * @since 4.0
 */
public class HotRodFastCopyHashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	public HotRodFastCopyHashMap(int initialCapacity, float loadFactor) {
	...
	public HotRodFastCopyHashMap(Map<? extends K, ? extends V> map) {
		if ( map instanceof HotRodFastCopyHashMap ) {
			HotRodFastCopyHashMap<? extends K, ? extends V> fast = (HotRodFastCopyHashMap<? extends K, ? extends V>) map;
	...
	public HotRodFastCopyHashMap(int initialCapacity) {
		this( initialCapacity, DEFAULT_LOAD_FACTOR );
	}

	public HotRodFastCopyHashMap() {
	...
	@Override
	@SuppressWarnings("unchecked")
	public HotRodFastCopyHashMap<K, V> clone() {
		try {
			HotRodFastCopyHashMap<K, V> clone = (HotRodFastCopyHashMap<K, V>) super.clone();
	...
	private abstract class HotRodFasyCopyHashMapIterator<E> implements Iterator<E> {

		private int next = 0;
		private int expectedCount = modCount;
		private int current = -1;
		private boolean hasNext;
		Entry<K, V> table[] = HotRodFastCopyHashMap.this.table;
		...
		public void remove() {
			...
			Entry<K, V>[] table = this.table;
			if ( table != HotRodFastCopyHashMap.this.table ) {
				HotRodFastCopyHashMap.this.remove( table[delete].key );
				table[delete] = null;
			...
			for ( ;; ) {
				...
				if ( ( i < prefer && ( prefer <= delete || delete <= i ) ) || ( prefer <= delete && delete <= i ) ) {
					...
					if ( i < current && current <= delete && table == HotRodFastCopyHashMap.this.table ) {
	...
	private class KeyIterator extends HotRodFasyCopyHashMapIterator<K> {
	...
	private class ValueIterator extends HotRodFasyCopyHashMapIterator<V> {
	...
	private class EntryIterator extends HotRodFasyCopyHashMapIterator<Map.Entry<K, V>> {
		...
		private class WriteThroughEntry extends AbstractMap.SimpleEntry<K, V> {
			...
			public V setValue(V value) {
				if ( table != HotRodFastCopyHashMap.this.table )
					HotRodFastCopyHashMap.this.put( getKey(), value );
	...
	public final class Values extends AbstractCollection<V> {
		...
		@Override
		public int size() {
			return HotRodFastCopyHashMap.this.size();
		}
		...
		@Override
		public void clear() {
			HotRodFastCopyHashMap.this.clear();
		}
	}
	...
	public class KeySet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			int size = size();
			HotRodFastCopyHashMap.this.remove( o );
			return size() < size;
		}
	...
	public class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		@Override
		public boolean isEmpty() {
			return HotRodFastCopyHashMap.this.isEmpty();
		}
