...
public class RemoteNeo4jEntityQueries extends BaseNeo4jEntityQueries {
	...
	private static final ClosableIteratorAdapter<NodeWithEmbeddeds> EMPTY_NODES = new ClosableIteratorAdapter<>( Collections.<NodeWithEmbeddeds>emptyList().iterator() );
	...
	public NodeWithEmbeddeds findEntity(RemoteNeo4jClient executionEngine, Long transactionId, Object[] columnValues) {
		...
		if ( queryResult != null ) {
			...
			return new NodeWithEmbeddeds( owner, embeddedNodesMap );
		}
		return null;
	}
	...
	public ClosableIterator<NodeWithEmbeddeds> findEntitiesWithEmbedded(RemoteNeo4jClient executionEngine, Long txId) {
		Statements statements = new Statements();
	 ...
	 */
	public ClosableIterator<NodeWithEmbeddeds> findEntities(RemoteNeo4jClient executionEngine, EntityKey[] keys, Long txId) {
	 ...
	 */
	private ClosableIterator<NodeWithEmbeddeds> multiPropertiesIdFindEntities(RemoteNeo4jClient executionEngine, EntityKey[] keys, Long txId) {
	 ...
	 */
	private ClosableIterator<NodeWithEmbeddeds> singlePropertyIdFindEntities(RemoteNeo4jClient executionEngine, EntityKey[] keys, Long txId) {
	...
	private ClosableIterator<NodeWithEmbeddeds> closableIterator(List<StatementResult> results) {
		return closableIterator( results, null );
	}

	private ClosableIterator<NodeWithEmbeddeds> closableIterator(List<StatementResult> results, EntityKey[] keys) {
		if ( results != null ) {
			...
			if ( data.size() > 0 ) {
				...
				if ( keys == null ) {
					List<NodeWithEmbeddeds> nodeWithEmbeddeds = new ArrayList<>();
					for ( Node owner : owners ) {
						nodeWithEmbeddeds.add( new NodeWithEmbeddeds( owner, nodes.get( owner.getId() ) ) );
					}
					return new ClosableIteratorAdapter<>( nodeWithEmbeddeds.iterator() );
				}
				else {
					NodeWithEmbeddeds[] array = new NodeWithEmbeddeds[keys.length];
					for ( Node owner : owners ) {
						int index = findKeyIndex( keys, owner );
						if ( index > -1 ) {
							array[index] = new NodeWithEmbeddeds( owner, nodes.get( owner.getId() ) );
						}
					}
					List<NodeWithEmbeddeds> nullRemoved = new ArrayList<>();
					for ( NodeWithEmbeddeds node : array ) {
						if ( node != null ) {
