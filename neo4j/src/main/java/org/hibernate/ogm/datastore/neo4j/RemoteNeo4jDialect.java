...
import org.hibernate.ogm.datastore.neo4j.remote.dialect.impl.NodeWithEmbeddeds;
...
public class RemoteNeo4jDialect extends BaseNeo4jDialect {
	...
	public Tuple getTuple(EntityKey key, TupleContext context) {
		...
		NodeWithEmbeddeds node = queries.findEntity( dataBase, txId, key.getColumnValues() );
		if ( node == null ) {
			return null;
		}
	...
	public List<Tuple> getTuples(EntityKey[] keys, TupleContext tupleContext) {
		...
		ClosableIterator<NodeWithEmbeddeds> nodes = entityQueries.get( metadata ).findEntities( dataBase, keys, txId );
		try {
	 ...
	 */
	private List<Tuple> tuplesResult(EntityKey[] keys, TupleContext tupleContext, ClosableIterator<NodeWithEmbeddeds> nodes, Long txId) {
		...
		while ( nodes.hasNext() ) {
			NodeWithEmbeddeds node = nodes.next();
	...
	public Association getAssociation(AssociationKey associationKey, AssociationContext associationContext) {
		...
		NodeWithEmbeddeds node = entityQueries.get( entityKey.getMetadata() ).findEntity( dataBase, transactionId, entityKey.getColumnValues() );
		if ( node == null ) {
			return null;
		}
	...
	public void forEachTuple(ModelConsumer consumer, TupleContext tupleContext, EntityKeyMetadata entityKeyMetadata) {
		...
		ClosableIterator<NodeWithEmbeddeds> queryNodes = entityQueries.get( entityKeyMetadata ).findEntitiesWithEmbedded( dataBase, txId );
		while ( queryNodes.hasNext() ) {
			NodeWithEmbeddeds next = queryNodes.next();
	...
	public ClosableIterator<Tuple> executeBackendQuery(BackendQuery<String> backendQuery, QueryParameters queryParameters, TupleContext tupleContext) {
		...
		if ( backendQuery.getSingleEntityMetadataInformationOrNull() != null ) {
			...
			ClosableIterator<NodeWithEmbeddeds> entities = entityQueries.get( entityKeyMetadata ).findEntities( dataBase, keys, txId );
