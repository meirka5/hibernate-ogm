...
public class MongoDBSessionSQLQueryTest extends OgmTestCase {
	...
	public void testIteratorSingleResultQuery() throws Exception {
		...
		Iterator<OscarWildePoem> result = session.createSQLQuery( nativeQuery )
	...
	@Test
	@Ignore
	public void testIteratorMultipleResultQuery() throws Exception {
		Session session = openSession();
		Transaction transaction = session.beginTransaction();
	...
	@Test
	@Ignore
	public void testListMultipleResultQueryReturningObject() throws Exception {
		Session session = openSession();
		Transaction transaction = session.beginTransaction();

		String nativeQuery = "{ $query : { author : 'Oscar Wilde' }, $orderby : { name : 1 } }";
		@SuppressWarnings("unchecked")
		List<Object> result = session.createSQLQuery( nativeQuery )
				.list();
